...
import { collection, getDocs, orderBy, query } from 'firebase/firestore'; // Alat penyedot data
import Link from 'next/link';
import { Search, MapPin, Phone, Menu, Facebook, Instagram, Bed, Bath, Move } from 'lucide-react';

export default function Home() {
  
  // Siapkan wadah untuk menampung data rumah asli
  const [properties, setProperties] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  // Fungsi: Ambil data dari Firebase saat website dibuka
  useEffect(() => {
    const fetchProperties = async () => {
      try {
        const q = query(collection(db, "properties")); // Ambil dari koleksi 'properties'
        const querySnapshot = await getDocs(q);
        const dataRumah = querySnapshot.docs.map(doc => ({
          id: doc.id,
          ...doc.data()
        }));
        setProperties(dataRumah); // Masukkan data ke wadah
      } catch (error) {
      ...
      <section className="py-12 px-4 max-w-7xl mx-auto bg-white rounded-3xl shadow-sm mb-12">
        <div className="flex justify-between items-end mb-8">
            <div>
                ...
                <p className="text-gray-500 text-sm mt-1">Property terbaru dari Gold Grandis</p>
            </div>
        </div>

        {/* Kalau Loading */}
        {loading && <p className="text-center text-gray-500">Sedang memuat data properti...</p>}

        {/* Kalau Tidak Ada Data */}
        {!loading && properties.length === 0 && (
          <div className="text-center p-10 bg-gray-50 rounded-xl">
            <p className="text-gray-500">Belum ada properti yang diupload.</p>
            <Link href="/admin" className="text-blue-600 underline text-sm mt-2 block">Upload di Admin</Link>
          </div>
        )}
